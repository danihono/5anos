...
export function criarMenina(aparencia = APARENCIA) {
  ...
  const TRECHOS = [
    ...
    { rCima: 0.212, rBaixo: 0.2, h: 0.22, de: -0.26, dz: -0.02, vaoTras: 0.62 },
  ];
  ...
  /* A barra, e o fim de uma discussão que já custou três versões.

     Eu tentei três vezes terminar o cabelo com cilindros soltos pendurados na
     borda. Separados, leem como dentadura; grossos o bastante para se tocar,
     leem como garras; e finos e compridos, que foi a última tentativa, o
     Daniel viu e disse na hora: "muito longo com tranças, não sei por quê".

     Ele tem razão e a imagem dele confirma: aquele cabelo acaba num CORTE, com
     um degrau e nada mais. Então é só um trecho curto e mais estreito fechando
     a coluna. Nenhuma mecha solta. */
  const remate = new Group();
  remate.position.y = -0.22;
  pai.add(remate);
  // afina pouco (0.20 → 0.188): afinando muito, a coluna arredonda a ponta e a
  // silhueta vira cápsula, quando na imagem dele ela é um retângulo de topo
  // redondo com os lados descendo quase verticais até a barra
  for (const geo of arcosDeCabelo(0.2, 0.188, 0.1, VAO_ROSTO, 1.45)) {
    malha(geo, matCabelo, remate, 0, -0.05, 0).scale.z = 0.86;
  }
  ...
  function atualizar(dt, e) {
    ...
    if (andando) {
      ...
      /* E o cotovelo dobrava para TRÁS. Com +1.25 o antebraço aponta para -z,
         que é atrás dela; de costas, as duas mãos apareciam por cima da
         mochila. Quem corre leva o antebraço para a frente.

         0.95 e não 1.15: somando com o ombro à frente (0.62), a 1.15 o braço
         esticava na horizontal e ela parecia estar pedindo carona. */
      bracoE.cotovelo.rotation.x = -0.95 - s * 0.26 * amp;
      bracoD.cotovelo.rotation.x = -0.95 + s * 0.26 * amp;
      /* O SINAL estava trocado, e estava desde sempre. O braço esquerdo fica em
         x = -0.195; girar +Z leva a ponta dele para +x, ou seja para DENTRO do
         corpo. Com 0.13 dava para não reparar; quando eu subi para 0.20 para
         "abrir" a manga, na verdade enfiei as duas mãos por cima da mochila.
         Negativo à esquerda e positivo à direita é que abre. */
      bracoE.g.rotation.z = -0.12;
      bracoD.g.rotation.z = 0.12;
    ...
    // tropeçando, os braços abrem de vez — mesmo sinal do resto: fora é
    // negativo à esquerda e positivo à direita
